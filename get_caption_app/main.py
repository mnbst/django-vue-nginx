...
notwant = ['-3fl9HVCk3o', 'vN_5MBIxjcs', 'KLFjdzvTLWM', 'CS9am2ctk1M', 'r9QG3eFJUJk', '1jQdXHdJIQw', 'FCeZqjTwO2M',
           ...
           '11I-WD-yBDU', 'TRsf-OH9rM4']

FireStore.delete_href(delete)
...
def fillindb(response):
    ...
    for search_result in response.get("items", []):
        ...
        cdata = None
        try:
            vdata = FireStore.video_data(href)
        except google.cloud.exceptions.NotFound:
            vdata is None
        try:
            cdata = FireStore.caption_data(href)
        except google.cloud.exceptions.NotFound:
            cdata is None

        if vdata._exists == True and cdata._exists == True and href not in renewal:
            continue
        ...
        if len(langcodes) < count_of_languagecode+1 and 'id' in langcodes:
            ...
            # set to firestore
            FireStore.set_caption(href, search_result, title, script, element)
            print(href)
...
def mkscript(name, href):
    ...
    for r in root.iter():
        ...
        for j, w in enumerate(word):
            ...
            if w != 'di' and j+1 != len(word) and word[j+1] != 'ini' and w != '' and word[j+1] != '' and word[j+1] != '-' and w != '-' and not regex.match(w) and not regex.match(word[j+1]):
                if regex.match(w):
                    print('error')
                i_word = w+' '+word[j+1]
                i_word = i_word.lower()
